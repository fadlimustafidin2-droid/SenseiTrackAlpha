...
(function () {
  ...
  let profile = ST.getAccountById(session.accountId);
  ...
  function paintDisplay() {
    ...
    document.getElementById('profileRoleBadge').textContent = profile.role === 'admin' ? 'Admin' : 'Guru';
    if (profile.avatar) {
  ...
  saveBtn.addEventListener('click', () => {
    ...
    if (ST.isEmailTaken(newEmail, session.accountId)) {
      App.showToast('Email tersebut sudah dipakai akun lain.', 'error');
      return;
    }
    profile = {
      ...profile,
    ...
    ST.updateAccount(session.accountId, profile);
    paintDisplay();
    setEditing(false);
