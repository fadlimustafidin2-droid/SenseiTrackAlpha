...
(function () {
  ...
  document.getElementById('greetingText').textContent = `${App.timeGreeting()}, ${(profile.name || 'Guru').split(',')[0].split(' ')[0]} 👋`;
  document.getElementById('greetingDate').textContent = App.formatDate(new Date().toISOString().slice(0, 10));

  const assessments = ST.getAssessments(); // sorted desc by date already
  ...
  ST.INDICATOR_BANK.forEach((c) => { categoryLabelMap[c.category] = c.label; });
  const catTotals = {};
  const catCounts = {};
  ...
  document.getElementById('statBestCategory').textContent = catAverages[0] ? categoryLabelMap[catAverages[0].cat] : '-';
  ...
  /* ---- Category doughnut ---- */
  const catOrder = ['pedagogik', 'profesional', 'kepribadian', 'sosial'];
  const catColors = ['#2563EB', '#14B8A6', '#F59E0B', '#8B5CF6'];
  const presentCats = catOrder.filter((c) => catTotals[c] !== undefined);
  new Chart(document.getElementById('categoryChart'), {
    type: 'doughnut',
    data: {
      labels: presentCats.map((c) => categoryLabelMap[c].replace('Kompetensi ', '')),
      datasets: [{
        data: presentCats.map((c) => +(catTotals[c] / catCounts[c]).toFixed(2)),
        backgroundColor: presentCats.map((c) => catColors[catOrder.indexOf(c)]),
        borderWidth: 0,
        hoverOffset: 6,
      }],
    },
